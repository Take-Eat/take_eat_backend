...
/**
  * Serviço para buscar uma Carteira pelo seu ID.
 \*
  * @async
  * @function getCarteiraIdService
  * @param {number} id - Identificação da Carteira para retornar seus dados.
  * @throws {AppError} Caso não seja encontrada a Carteira.
  * @returns {Promise<iCarteira>} A Carteira encontrada e validada.
 \*
  * @example
  * // Exemplo de chamada
  * const retrivedCarteira = await getCarteiraIdService(2);
  * // Exemplo de retorno
 * {
 *   id: 2,
 *   totalDoado: 200.50,
...
const getCarteiraIdService = async (id: number): Promise<iCarteira> => {
...
export default getCarteiraIdService;
